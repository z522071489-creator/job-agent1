...
# 二次筛选排除条件（出现这些关键词的岗位会被标记为不符合）
SECONDARY_FILTER_EXCLUDE_KW = [
    "犯罪记录", "被开除", "失信", "信用问题", "债务", "限制高消费",
    "受过刑", "被判刑", "党纪处分", "政纪处分", "限制出国", "失业",
    "服刑期间", "有期徒刑", "无期徒刑", "违法犯罪",
]
...
4. **必须没有不合理的限制条件**
   - 排除：有犯罪记录、信用问题、债务问题、被开除、失信等排除性条件的岗位
...
def analyze_with_kimi(article: dict, apply_links: list[str], apply_page_contents: list[str]) -> list[dict]:
    ...
    if apply_page_contents:
        apply_info += "\n\n【投递页面要求摘要】\n" + "\n".join(apply_page_contents[:3])  # 只取前3个
    ...
    user_msg = (
        ...
        f"如果发现投递页面或文章中有不合理的限制条件（如犯罪记录、信用问题等），必须标记为excluded=true。"
...
def secondary_filter_jobs(jobs: list[dict], article_content: str, apply_contents: list[str]) -> list[dict]:
    """
    二次筛选：检查文章内容和投递页面，排除有不合理限制条件的岗位
    """
    # 合并所有要检查的内容
    all_text = article_content + "\n" + "\n".join(apply_contents)
    ...
    for job in jobs:
        ...
        # 检查排除关键词
        for kw in SECONDARY_FILTER_EXCLUDE_KW:
            if kw in all_text_lower:
                excluded = True
                exclude_reason = f"发现限制条件: {kw}"
                print(f"    ⚠️ 二次筛选排除: {job.get('job_name')} - {exclude_reason}")
                break
...
def main():
    ...
    for i, art in enumerate(filtered, 1):
        ...
        # 第三步：获取投递页面内容（用于二次筛选）
        apply_page_contents = []
        for link in all_apply_links[:2]:  # 只获取前2个链接的内容
        ...
        if jobs:
            print(f"   🔍 执行二次筛选...")
            jobs = secondary_filter_jobs(jobs, content, apply_page_contents)
